...
def get_user_by_id(
    ...
    if (
        current_user.role == UserRoleType.SAO_ADMIN
        or current_user.role == UserRoleType.CLUB_MANAGER
    ):  # type: ignore
...
def create_user(user: UserCreate, db: Session = Depends(get_db)):
    ...
    db_user = UserModel(
        **user.model_dump(exclude={"password", "role"}), hashed_password=hashed_password
    )  # Modified
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
...
def get_clubs_for_user(
    ...
    clubs = (
        ...
        .filter(club_memberships.c.user_id == user_id)
        .all()
    )
    return clubs
...
def get_events_attended_by_user(
    ...
    events = (
        ...
        .filter(event_attendance.c.user_id == user_id)
        .all()
    )
    return events
