...
def get_event_by_id(event_id: int, db: Session = Depends(get_db), current_user : UserModel = Depends(get_current_user)):
    ...
    if(event.status.value == EventStatusType.IDEATION  ):
        if is_event_owner:
            ...
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Event access unothorized")
    if (event.status.value == EventStatusType.PLANNING):
...
def register_user_for_event(
    ...
    is_club_manager = False
    if is_student or is_admin:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to add attendance for this event")
