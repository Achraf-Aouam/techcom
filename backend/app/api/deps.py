...
def get_current_user(
    ...
    user = (
        db.query(UserModel).filter(UserModel.email == token_data.username).first()
    )  # Or student_id based on what's in 'sub'
    if user is None:
        user = (
            db.query(UserModel)
            .filter(UserModel.student_id == token_data.username)
            .first()
        )
        if user is None:
            raise credentials_exception
